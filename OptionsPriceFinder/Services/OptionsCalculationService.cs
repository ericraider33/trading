...
public class OptionsCalculationService
{
    ...
    public async Task<OptionResult> CalculateOptionsForSymbolAsync(string symbol)
    {
        try
        {
            ...
            var stockPrice = await _stockService.getCurrentPrice(symbol);
            ...
            var option1Percent = await _optionsService.getCallOptionPrice(symbol, nextFriday, price1Percent);
            var option2Percent = await _optionsService.getCallOptionPrice(symbol, nextFriday, price2Percent);
            var option3Percent = await _optionsService.getCallOptionPrice(symbol, nextFriday, price3Percent);
