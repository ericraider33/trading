...
public class PolygonApiClient
{
    ...
    public PolygonApiClient(HttpClient httpClient, string apiKey)
    {
        ...
        this.apiKey = apiKey;
        this.httpClient.BaseAddress = new Uri("https://api.polygon.io/");
    }
    ...
    public async Task<StockPrice> GetCurrentStockPriceAsync(string symbol)
    {
        try
        {
            var response = await httpClient.GetAsync($"v2/last/trade/{symbol}?apiKey={apiKey}");
    ...
    public async Task<List<OptionData>> GetOptionsDataAsync(string symbol, DateTime expirationDate, decimal targetPrice)
    {
        try
        {
            ...
            var response = await httpClient.GetAsync(
                $"v3/reference/options/contracts?underlying_ticker={symbol}&expiration_date={formattedDate}&apiKey={apiKey}");
            response.EnsureSuccessStatusCode();
            ...
            foreach (var contract in callOptions)
            {
                try
                {
                    var priceResponse = await httpClient.GetAsync(
                        $"v2/last/trade/O:{contract.Ticker}?apiKey={apiKey}");
